...
export class Serializer {
    ...
    static serializeWTx(tx: Transaction) {
        ...
        for (let i = 0; i < tx.vin.length; i++) {
            ...
            if (
                ...
                serializedWTx += compactSize(
                    BigInt(input.witness.length)
                ).toString("hex");
                for (const witness of input.witness) {
                    serializedWTx += compactSize(
                        BigInt(witness.length / 2)
                    ).toString("hex");
    ...
    static serializeWitness(tx: Transaction, index: number, sighash: SigHash) {
        //pubkey is tx.input[vout].witness[1] for p2wpkh
        if (sighash !== SigHash.ALL) throw new Error("unsupported sighash");
        let serializedWTx = "";
        ...
        for (const output of tx.vout) {
            ...
            outputAmount.writeBigInt64LE(BigInt(output.value), 0);
            outputs += outputAmount.toString("hex");
        ...
        // const outpoint = input.txid;
        const vout = Buffer.alloc(4);
        vout.writeUint32LE(input.vout, 0);
        const outpoint = input.txid + vout.toString("hex");
        ...
        hashcode.writeUint16LE(0x01, 0);

        console.log("version: ", version.toString("hex"));
        console.log("hashPrevouts: ", hashPrevouts, "prevouts: ", prevouts);
        console.log("hashSequence: ", hashSequence, "sequences: ", sequences);
        ...
        console.log("hashOutputs: ", hashOutputs, "outputs: ", outputs);
    ...
    vin: [
        {
            ...
            prevout: {
                ...
                scriptpubkey_address:
                    "bc1qlrvlygpudurh8xpnj2jg04zupjqcl9tnk5np40",
                value: 37079526,
            },
            scriptsig: "",
            scriptsig_asm: "",
